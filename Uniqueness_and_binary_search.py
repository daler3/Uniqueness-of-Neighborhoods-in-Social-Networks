...
""" This code computes the average degree (average number of edges per node) of a random graph/network corresponding to a given percentage of unique neighborhoods structures ("uniqueness").
It does that with the function called "binarySearchUnique". There is also a main with an example of call of the function. The other functions are auxiliary. 
To estimate the average degree of a graph corresponding to a percentage of unique neighborhoods, the code generates random graphs and computes the mean and the standard deviation of the obtained uniqueness.
A neighborhood is the subgraph of a graph that contains the neighbors of a node and the edges between those (but does not contain the central node itself). 
A neighborhood structure is unique if there are no other neighborhoods in the original graph that have its same structure (i.e. that are not isomorphic). 
The considered graphs are undirected (the edges have no direction), unlabelled (the nodes have no label), without self loops and without multi-edges (multiple edges between a pair of nodes). 
To handle graphs, Pymnet, a multilayer network library, is used. Thanks to this, one can extend the code to handle graphs with multiple layers. To compute isomorphism, Pymnet uses PyBliss (http://www.tcs.hut.fi/Software/bliss/).
It is assumed that the given interval for binary search (continuos version, that just needs the extreme values) contains the value we are looking for (otherwise the method fails).
Large-enough networks in their sparse-region (such as with relatively small average degree) are considered in this problem. 
Empirical evaluations have shown that higher the average degree, higher the number of unique neighborhoods (this is not necessarily true in the graph dense region, i.e. when the graph is almost complete).
This last consideration is crucial in order to make a decision regarding which new interval to consider in the recursion of the binary search.
""" 
...
def compute_uniqueness(net):
	...
	for k in dic_count_n.keys():
		if dic_count_n[k] == 1: 
			count_n += 1 
	return float(count_n) / float(len(list(net))) #dividing the number of unique neighborhoods by the number of nodes in the network (such as the total number of neighborhoods)
...
def containedInInterval(value, low, up, tolerance=0):
	""" This function returns True if the given value (plus or minus a tolerance) is contained in the interval delimited by low and up, and False otherwise.
	...
	if tolerance == 0: 
		...
		return containedInInterval(value, low, up) or containedInInterval(value-tolerance, low, up) or containedInInterval(value+tolerance, low, up)
...
def binarySearchUnique(lowervalue, uppervalue, uniqval, size, name_model, model_par=None, tolerance_threshold=0.05, n_decisions=0, z_value=2.58, single_sim_number=10, max_sim=50):
	""" This function performs a binary search (continuos version), looking for an average degree value for a given network model, with which the network 
	would have a certain percentage of unique neighborhoods' structure (with a given tolerance threshold). The delimiting average degree values are given. 
	To estimate the uniqueness value in a network model corresponding to a given average degree, the function performs simulations by generating random graphs according to the specified model.

	Parameters
	----------
	lowervalue : int or float
		lower value of average degree delimiting the interval in which the binary search is performed 
	uppervalue : int or float
		upper value of average degree delimiting the interval in which the binary search is performed 
	size : int 
		network size (number of nodes in the graph)
	name_model : str
		name of the graph model we want to evaluate
	model_par : list 
		parameters of the graph model
	uniqval : float
		the target uniqueness value 
	tolerance_threshold: float
		the tolerance value for uniqueness (e.g. if we are looking for 0.5 uniqueness, 0.5-tolerance_threshold or 0.5+tolerance_threshold are also fine)
	n_decisions : int
		number of decisions already taken in the previous calls of the algorithm
	z_value: float
		z-value corresponding to a certain confidence level (the confidence level we want to make decisions with)
	single_sim_number : int
		number of initial simulations (graphs generation) to perform for each degree value's evaluation
	max_sim : int 
		maximum simulation numbers for each degree value to evaluate
	...
	Notes
	------
	The method can fail if the given extremes value are not valid or if the degree value corresponding to the targeted uniqueness is not found. 
	In this case it returns -1, -1, -1, -1.
	The target uniqueness value should be greater than 0 and lower than 1, since there is a wide region where the graph has no or all unique neighborhoods, 
	and a single average degree value cannot be determined.
	""" 
	
	def doSimulations(deg, sim_number=single_sim_number, simulation_list=[]):
		""" This function performs the simulation (i.e. generates a network with a given average degree) a specified number of times and compute 
		the number of unique neighborhoods for each of those. It returns the mean and the confidence interval of the results. 
		If previous simulation were carried out, a list with the corresponding uniqueness results can be passed as a parameter
		"""
	...
	def evaluate(deg, sim_number=single_sim_number, simulation_list=[]): 
		""" This function computes the uniqueness of a network with a given average degree and checks if more simulation
		are needed or not, in order to be confident (at 99% confidence level) that the value of the average degree is or is not 
		the one we are looking for.
		"""
		mean, mean_conf_lower, mean_conf_upper = doSimulations(deg, sim_number=sim_number, simulation_list=simulation_list)
	...
	def endEvaluation(mean_conf_lower, mean_conf_upper, middlevalue, n_decisions, lowervalue, uppervalue):
		""" This function evaluates if a result reached at the end of the searching process is acceptable or not. 
		The function is called when the maximum number of simulations has been reached or when the two extremes values are
		close to each other. If the uniqueness value (with the tolerance level) is within the confidence interval of the so far computed mean,
		then the result is acceptable. Otherwise the method fails. 
		"""
		#if the target value (plus or minus the tolerance) is contained in the interval, then return the middle value, otherwise the method fails
	...
	if lowervalue < 0 or uppervalue < 0 or uppervalue < lowervalue or uppervalue > (size-1): #the uppervalue cannot be greater than the maximum allowed degree
	...
	if (uppervalue - lowervalue) < 0.02: 
		mean, mean_conf_lower, mean_conf_upper = doSimulations(middlevalue, sim_number=single_sim_number) #simulate with the middle value
	...
	meanvalue, mean_conf_lower, mean_conf_upper, reachedLimit = evaluate(middlevalue, simulation_list=[]) #evaluate the middle value
	...
	if containedInInterval(meanvalue, uniqval-tolerance_threshold, uniqval+tolerance_threshold):
		...
		return binarySearchUnique(low_extreme, up_extreme, uniqval, size, name_model, model_par=model_par, n_decisions=n_decisions+1, z_value=z_value)
...
if __name__ == "__main__":
	...
	deg, n_decisions, lowervalue, uppervalue = binarySearchUnique(deg_low_value, deg_up_value, target_uniqueness_value, net_size, name_model, z_value=z_value)
	print "The average degree value that gives a "+str(name_model)+" network with "+str(net_size)+" nodes and with "+str(target_uniqueness_value)+" uniqueness is: "+str(deg)
